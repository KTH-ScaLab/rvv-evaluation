...
def repeat_2_30(n):
    return [2 + (i % 29) for i in range(n)]
...
def generate_block(lines, clobber, block_ops, blk, lanes, init, init_mem, check_reg):
    ...
    for i in range(count):
        ...
        lines.append('%s"%s %s, (%%0), v0.t %s \\n\\t"' % (indent, opcode, operands[0], ", ".join(operands[1::1])))
    if blk.isa == "rv64gcv":
        lanes.append("((int)8*__riscv_vlenb()*%s/%s)" % (lmuls_to_int(blk.lmul), blk.typebits))
        print(blk.maskcode)
        if blk.maskcode == 1010:
            init.append(f"""
    ...
    asm volatile ("vid.v      v1\\n\\t"
                  ...
                  : "v0", "v1");
        """)
        elif blk.maskcode > 16:
            init.append(f"""
             asm volatile ("vsetvli %0, %1, e{blk.typebits}, {blk.lmul}, {blk.tail}, {blk.mask}": "=r"(avl): "r"(lanes): "vl", "vtype");
             long limit = {blk.maskcode};
    //printf("The active vector length of RVV is %d\\n", avl);
    asm volatile (
    "vid.v       v1\\n\\t"
    "vmsltu.vx   v0, v1, %[limit]\\n\\t"
    :
    : [limit] "r" (limit)
    : "v0", "v1"
);
    ...
    asm volatile ("vid.v      v1\\n\\t"
        ...
        lanes.append("1")
    if blk.maskcode == 1010:
        block_ops.append("(%s*(%s*lanes))/2" % (blk.count, blk.laneops))
    else:
        block_ops.append("(%s*(%s*%s))" % (blk.count, blk.laneops, blk.maskcode))
...
def parse_args(args):
    ...
    block_parser.add_argument("maskcode", type=int, help="The value of the mask register")
